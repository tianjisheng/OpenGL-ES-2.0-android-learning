...
class MatrixRender : GLSurfaceView.Renderer {
    ...
    override fun onSurfaceCreated(gl: GL10?, config: EGLConfig?) {
        ...
        var matrixBuffer: FloatBuffer = FloatBuffer.allocate(xformMatrix.size)
        matrixBuffer.put(xformMatrix)
        ...
        val u_xformMatrix = GLES20.glGetUniformLocation(mProgram, "u_xformMatrix")
        GL2Utils.checkGlError("uniform loaction")
        LogUtils.i("u_xformMatrix == $u_xformMatrix,buffer = ${matrixBuffer[15]}")
        GLES20.glUniformMatrix4fv(u_xformMatrix,16,false,matrixBuffer)
